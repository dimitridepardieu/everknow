import { createFileRoute, useNavigate } from "@tanstack/react-router";
import { Button } from "@/components/ui/button";

export const Route = createFileRoute("/onboarding/account-type")({
	component: AccountTypePage,
});

function AccountTypePage() {
	const navigate = useNavigate();

	return (
		<div className="flex min-h-svh flex-col items-center justify-center gap-8 px-6">
			<p>Comment allez-vous utiliser Flashcard Academy ?</p>

			<div className="flex w-full max-w-sm flex-col gap-3">
				<Button
					variant="outline"
					className="w-full cursor-pointer"
					onClick={() => navigate({ to: "/onboarding/create-profile" })}
				>
					👨‍👩‍👧 Je suis un parent
				</Button>
				<Button
					variant="outline"
					className="w-full cursor-pointer"
					onClick={() => navigate({ to: "/onboarding/first-deck" })}
				>
					🎓 J'apprends pour moi
				</Button>
			</div>
		</div>
	);
}
